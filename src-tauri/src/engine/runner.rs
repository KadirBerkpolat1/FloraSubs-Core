        ...
        if let Some(job) = job_opt {
            ...
            #[cfg(target_os = "windows")]
            {
                let _ = std::process::Command::new("taskkill")
                    .args(["/F", "/T", "/PID", &job.pid.to_string()])
                    .output();
            }
...
pub async fn start_encoding_job(
    ...
    if config.hardsub_enabled {
        if config.subtitle_source == "embedded" {
            ...
            extract_subtitle_track(&config.input_path, sub_idx, &temp_sub)?;
        ...
        Ok(status) => {
            if status.success() {
                ...
                let _ = app.emit(
                    ...
                    JobLogMessage {
                        job_id: job_id.clone(),
                        line: "[FloraSubs] Kodlama başarıyla tamamlandı!".to_string(),
                        stream: "system".to_string(),
                        timestamp: chrono::Local::now().format("%H:%M:%S").to_string(),
                    },
                );
                ...
                let _ = app.emit(
                    ...
                    JobLogMessage {
                        job_id: job_id.clone(),
                        line: format!("[FloraSubs] HATA: FFmpeg çıkış kodu {}", code),
                        stream: "stderr".to_string(),
                        timestamp: chrono::Local::now().format("%H:%M:%S").to_string(),
                    },
                );
